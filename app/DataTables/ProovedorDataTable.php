...
class ProovedorDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function (Proveedor $proveedor) {
                $id = $proveedor->id;
                return view('proveedores.datatables_actions', compact('proveedor','id'));
            })
            ->editColumn('id', fn (Proveedor $proveedor) => $proveedor->id)
            ->rawColumns(['action']);
    }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->responsive(true)
            ->stateSave(false)
            ->orderBy(0, 'desc') // ordenar por la primera columna (id)
            ->dom('
                <"row mb-2"
            ...
            ->buttons(
                ...
                Button::make('export')->extend('collection')->text('<i class="fa fa-download"></i> <span class="d-none d-sm-inline">Exportar</span>')
                    ->buttons([
    ...
    {
        return [
            Column::make('id')->title('ID')->addClass('text-center')->width('10%'),
